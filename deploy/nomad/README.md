...
`ramble.nomad.hcl` is the production deployment: Ramble on the the hosting provider Nomad
cluster (`the-cluster`), behind the platform Traefik, on the platform Postgres.
...
| 1Password `Infra / ramble-env` | the same values, source of truth |
| 1Password `Infra / postgres-ramble` | the database role |
...
Manual deploy from a private network device:

    export NOMAD_ADDR=http://the-cluster:4646 NOMAD_TOKEN=$(op read "op://<vault>/operator-token/credential")
    nomad job run -var image=ghcr.io/open-wander/ramble:0.5.11 deploy/nomad/ramble.nomad.hcl

Platform side (Traefik, Postgres, backups, logs): `the platform repo` `platform/`.
