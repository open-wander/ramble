...
func fetchGitHubMetadata(repoURL string, token string) (*GitHubRepo, error) {
	...
	resp, err := client.Do(req) // #nosec G704 -- URL constructed from validated github.com host, client is SSRF-protected
	if err != nil {
...
func fetchGitLabMetadata(repoURL string, token string) (*GitLabProject, error) {
	...
	resp, err := client.Do(req) // #nosec G704 -- URL constructed from validated gitlab.com host, client is SSRF-protected
	if err != nil {
